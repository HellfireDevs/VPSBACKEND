...
engine = create_engine(
    ...
    pool_recycle=1800,      # 30 min baad refresh
)
...
class VPSOrder(Base):
    ...
    # AWS Details
    instance_id         = Column(String(50), unique=True, nullable=True)   # i-xxxxxxxx
    instance_type       = Column(String(50), nullable=False)               # m7i-flex.large
    os                  = Column(String(100), nullable=False)              # Ubuntu 22.04
...
class Trial(Base):
    ...
    ip_address      = Column(String(45), nullable=False)    # IP lock
    aws_account_id  = Column(Integer, ForeignKey("aws_accounts.id"), nullable=True)
    started_at      = Column(DateTime, default=datetime.utcnow)
    expires_at      = Column(DateTime, nullable=False)
    is_used         = Column(Boolean, default=True)         # Ek IP ek trial
